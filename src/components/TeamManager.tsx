...
interface Team {
  ...
  is_active: boolean;
  created_at: string;
  members?: Array<{ id: number; name: string; team_role?: string; skills?: string[] }>;
}
...
export function TeamManager() {
  ...
  useEffect(() => {
    if (activeTab === 'members' && cachedMembers) {
      setTeamMembers(cachedMembers as TeamMember[] || []);
      setFilteredMembers(cachedMembers as TeamMember[] || []);
      return;
    }
    ...
    fetchData();
  }, [activeTab, cachedMembers, cachedTeams]);
  ...
  useEffect(() => {
    const fetchSkills = async () => {
      try {
        ...
        const skillNames = skillsArray.map((skill: unknown): string => {
          if (typeof skill === 'string') return skill;
          const maybe = skill as { name?: string };
          return maybe.name || '';
  ...
  useEffect(() => {
    if (activeTab === 'members') {
      const filtered = teamMembers.filter(member =>
        ...
        (member.skills && member.skills.some(skill => skill.toLowerCase().includes(debouncedSearch.toLowerCase())))
      );
      ...
      const filtered = teams.filter(team =>
        ...
        (team.description && team.description.toLowerCase().includes(debouncedSearch.toLowerCase())) ||
        (team.skills && team.skills.some(skill => skill.toLowerCase().includes(debouncedSearch.toLowerCase())))
      );
      setFilteredTeams(filtered);
      setVisibleTeams(24);
    }
  }, [debouncedSearch, teamMembers, teams, activeTab]);
  ...
  const handleUpdateMember = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updateData: Record<string, unknown> = { ...memberFormData };
      if (!updateData.passcode || (typeof updateData.passcode === 'string' && updateData.passcode.trim() === '')) {
  ...
  const handleViewMemberDetails = (member: { id: number; name: string; email?: string; team_role?: string; skills?: string[] }) => {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="border-b border-gray-200">
        <nav className="-mb-px flex space-x-8">
          <button
            ...
            className={`py-2 px-1 border-b-2 font-medium text-sm flex items-center space-x-2 ${activeTab === 'members'
          ...
          <button
            ...
            className={`py-2 px-1 border-b-2 font-medium text-sm flex items-center space-x-2 ${activeTab === 'teams'
                ? 'border-blue-500 text-blue-600'
                : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
              }`}
          >
      ...
      <div className="flex justify-center">
        {activeTab === 'members' && filteredMembers.length > visibleMembers && (
          <Button variant="outline" onClick={() => setVisibleMembers(v => v + 24)}>Load more members</Button>
        )}
        {activeTab === 'teams' && filteredTeams.length > visibleTeams && (
          <Button variant="outline" onClick={() => setVisibleTeams(v => v + 24)}>Load more teams</Button>
        )}
      </div>
            ...
            <div>
              ...
              {selectedTeam.members && selectedTeam.members.length > 0 ? (
                <div className="space-y-2 mt-1">
                  {selectedTeam.members.map((member: { id: number; name: string; team_role?: string; skills?: string[] }) => (
